...
[Generator]
public class IterationGenerator : IncrementalGeneratorBase<IterationGenerator.Info> {

    private const string GenerateIteration = nameof(GenerateIteration);
    private const string GenerateIterationAttribute = nameof(GenerateIterationAttribute);


    // ReSharper disable once InconsistentNaming
    private const string IComponentArrayIterationExpression =
        nameof(IComponentArrayIterationExpression);


    private const string IterationExpression = nameof(IterationExpression);
    ...
    protected override bool Choose(SyntaxNode node, CancellationToken token) {
        ...
        return attributeSyntax.Name.ExtractName()
            is GenerateIteration or GenerateIterationAttribute;
    }
    ...
    protected override Info? Select(
        ...
        var structSyntax = (StructDeclarationSyntax?)attribute.Parent?.Parent;
        ...
        var interfaceSymbol = structSymbol
            .Interfaces
            .FirstOrDefault(x => x.Name == IComponentArrayIterationExpression);

        if (interfaceSymbol == null) {
            return null;
        }

        var interfaceMember = interfaceSymbol.GetMembers(IterationExpression).First();
        var structMember =
            structSymbol.FindImplementationForInterfaceMember(interfaceMember);
        ...
        var methodSyntax =
            (MethodDeclarationSyntax)structMember.DeclaringSyntaxReferences.First()
                .GetSyntax();

        var source =
            GenerateSourceFile(methodSyntax, () => GenerateMethod(methodSyntax));

        return new Info($"{structSyntax.Identifier}.g.cs", source);
    }
    ...
    private static string GenerateMethod(MethodDeclarationSyntax methodSyntax) {
        ...
        source.Insert(0,
            $"{string.Join(" ", methodSyntax.Modifiers)} {methodSyntax.ReturnType} {methodSyntax.Identifier}_Generated{parameterList} ");
    ...
    private static string GenerateComponentArrayForEach(
        ...
        foreach (var componentParameter in lambda.ParameterList.Parameters) {
    ...
    if ({{arrayName}}.Archetype.Contains<{{componentsStr}}>()) {
        ...
        {{
            string.Join(" ", readComponents.Select(
                    x => $"var _span_{x.Identifier} = {arrayName}.GetReadOnlySpan<{x.Type}>();")
                .Concat(writeComponents.Select(
                    x => $"var _span_{x.Identifier} = {arrayName}.GetSpan<{x.Type}>();")))
        }}

        for (var _i = 0; _i < _count; ++_i) {
            {{
                string.Join(" ", readComponents.Select(
                        x => $"ref readonly var {x.Identifier} = ref _span_{x.Identifier}[_i];")
                    .Concat(writeComponents.Select(
                        x => $"ref var {x.Identifier} = ref _span_{x.Identifier}[_i];")))
            }}
